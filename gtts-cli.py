...
def text_to_speech(text, output_name="output", lang="en", format="mp3", voice=None):
    ...
    if voice:
        ...
        os.system(
            f"ffmpeg -i {output_name}.{format} -f mp3 -ab 192000 -ar 44100 {output_name}_voice.mp3"
        )
        os.remove(f"{output_name}.{format}")
        os.system(
            f"ffmpeg -i {output_name}_voice.mp3 -acodec pcm_u8 -ar 22050 {output_name}.wav"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--format", type=str, default="mp3", help="Output file format (default: mp3)"
    )
